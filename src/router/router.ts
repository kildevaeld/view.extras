...
export class BaseRouter extends EventEmitter {
    ...
    route(route: RegExp | string, name: RouteHandler | string, handler: RouteHandler = null) {
    ...
    protected execute(callback: RouteHandler, name: string, route: string | RegExp, args: any[]) {
        if (callback) {
            if (this.options.execute) {
                this.options.execute(callback, name, args)
            } else {
                callFunc(callback, this, [route].concat(args))
            }
        }
    }
